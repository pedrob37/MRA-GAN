...
class nnUNet(nn.Module):
    ...
    def expansive_block(self, in_channels, mid_channel, final_channel, kernel_size=3, block_dropout=0.0):
        ...
        block = torch.nn.Sequential(
                ...
                torch.nn.LeakyReLU(),
                torch.nn.Dropout(block_dropout).train(True),
                # Upsample(scale_factor=2, mode='pixelshuffle', align_corners=True),
                torch.nn.Upsample(scale_factor=2, mode='nearest', align_corners=True),
        )
        return block

    def subpixel_expansive_block(self, in_channels, mid_channel, final_channel, kernel_size=3, block_dropout=0.0):
        """
        This function creates one expansive block
        """
        block = torch.nn.Sequential(
    ...
    def __init__(self, in_channel, out_channel, dropout_level=0.0, z_concat_flag=False, z_output=0, final_act=torch.nn.LeakyReLU()):
        ...
        self.conv_decode1 = self.subpixel_expansive_block(120, 60, 30, block_dropout=dropout_level)
