...
class GANLoss(nn.Module):
    ...
    def __call__(self, input, target_is_real):
        #target_tensor = self.get_target_tensor(input, target_is_real) # Chin commented 2022.01.28
        target_tensor = self.get_target_tensor(input, target_is_real).to(device) # Chin added 2022.01.28
...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input):
        #device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # chin added 2022.01.28
        #input.to(device) # chin added 2022.01.28
