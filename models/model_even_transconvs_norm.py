...
class nnUNet(nn.Module):
    ...
    def penultimate_block(self, in_channels, mid_channel, kernel_size=3, block_dropout=0.0):
        ...
        """
        block = torch.nn.Sequential(
                torch.nn.Conv3d(kernel_size=kernel_size, in_channels=in_channels, out_channels=mid_channel, padding=1, stride=1, dilation=1),
                torch.nn.InstanceNorm3d(mid_channel),
                torch.nn.LeakyReLU(),
                # torch.nn.BatchNorm3d(mid_channel),
                torch.nn.Conv3d(kernel_size=kernel_size, in_channels=mid_channel, out_channels=mid_channel, padding=1, stride=1, dilation=1),
                torch.nn.InstanceNorm3d(mid_channel),
                torch.nn.LeakyReLU(),
                torch.nn.Dropout(block_dropout).train(True),
                )
    ...
    def __init__(self, in_channel, out_channel, dropout_level=0.0, z_concat_flag=False, z_output=0, final_act=torch.nn.LeakyReLU()):
        ...
        self.bottleneck = torch.nn.Sequential(
                            torch.nn.Conv3d(kernel_size=3, in_channels=240+z_output, out_channels=480, padding=1, stride=1, dilation=1),
    ...
    def forward(self, x, z_input=None):
        ...
        # print(f'x17 shape is 1 {final_layer.shape}')
        return final_layer
