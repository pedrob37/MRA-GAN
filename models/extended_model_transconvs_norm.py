...
class nnUNet(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, dropout_level=0.0, z_concat_flag=False, z_output=0, final_act=torch.nn.LeakyReLU()):
        ...
        self.conv_encode5 = self.contracting_block(240, 480, block_dropout=dropout_level)
        self.conv_maxpool5 = torch.nn.MaxPool3d(kernel_size=2)

        # Bottleneck
        self.bottleneck = torch.nn.Sequential(
                            torch.nn.Conv3d(kernel_size=3, in_channels=480+z_output, out_channels=960, padding=1),
                            torch.nn.InstanceNorm3d(960),
                            torch.nn.LeakyReLU(),
                            # torch.nn.BatchNorm3d(512),
                            torch.nn.ConvTranspose3d(in_channels=960, out_channels=480, kernel_size=3, stride=2,
        ...
        # Decode
        self.conv_decode4 = self.expansive_block(960, 480, 240, block_dropout=dropout_level)
    ...
    def forward(self, x, z_input=None):
        ...
        if self.z_concat_flag:
            # print(z_input.shape)
            encode_pool5 = self.tiling_and_concat(z_input, encode_pool5)

        # Bottleneck
        bottleneck1 = self.bottleneck(encode_pool5)
        # print(f'x9 shape is BN VIP 240 {bottleneck1.shape}')
        # Decode: Start with concat
        decode_block5 = self.crop_and_concat(bottleneck1, encode_block5, crop=False)
        # print(f'bottleneck shape is {bottleneck1.shape} and the encode block shape is {encode_block4.shape}')
        cat_layer4 = self.conv_decode4(decode_block5)
        # print(f'bottleneck shape is {bottleneck1.shape} and the encode block shape is {encode_block4.shape}')
        decode_block4 = self.crop_and_concat(cat_layer4, encode_block4, crop=False)
