...
class SSIM(torch.nn.Module):
    ...
    def __init__(
        ...
        self.k = k
        self.scales = scales
        self.multi_scale_weights = (
            torch.FloatTensor(multi_scale_weights) if multi_scale_weights else None
        )
        ...
        self.gradient_kernels = self._sobel_kernels()
    ...
    def gradient_map(self, input: torch.Tensor) -> torch.Tensor:
        ...
        for gk in self.gradient_kernels:
    ...
    def _ssim(self, original_images: torch.Tensor, perturbed_images: torch.Tensor):
        ...
        if self.star_based:
            denominator_mask = torch.logical_not(
                torch.logical_or(
                    luminance_denominator < self.star_thresholds[0],
                    luminance_denominator > self.star_thresholds[0],
                )
            ).float()
        ...
        if self.star_based:
            denominator_mask = torch.logical_not(
                torch.logical_or(
                    contrast_denominator < self.star_thresholds[0],
                    contrast_denominator > self.star_thresholds[0],
                )
            )
        ...
        if self.gradient_masks_weights:
            ...
            usable_masks = np.array([(gm == 1).any().numpy() for gm in gradient_masks])
