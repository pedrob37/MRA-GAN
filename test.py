...
def prepare_batch(image, ijk_patch_indices):
    ...
    for batch in ijk_patch_indices:
        image_batch = []
        for patch in batch:
            image_patch = image[int(patch[0]):int(patch[1]), int(patch[2]):int(patch[3]), int(patch[4]):int(patch[5])]
...
def inference(model, image_path, result_path, resample, resolution, patch_size_x,
    ...
    for i in tqdm(range(len(batches))):
        ...
        istart = int(ijk_patch_indices[i][0][0])
        iend = int(ijk_patch_indices[i][0][1])
        jstart = int(ijk_patch_indices[i][0][2])
        jend = int(ijk_patch_indices[i][0][3])
        kstart = int(ijk_patch_indices[i][0][4])
        kend = int(ijk_patch_indices[i][0][5])
    ...
    label_np = label_np[:int(pad_x), :int(pad_y), :int(pad_z)]
